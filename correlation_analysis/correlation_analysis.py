...
def parse_args():
    ...
    parser.add_argument(
        "--artist", type=str, help="Name of the artist to process"
    )
    parser.add_argument(
        "--song",
        type=str,
        help="Name of the song to process (requires --artist)",
    )
    parser.add_argument(
        "--save_strong_windows",
        action="store_true",
    ...
    for artist in tqdm(os.listdir(dataset_path), desc="Artists"):
        ...
        for song in tqdm(os.listdir(artist_dir), desc="Songs", leave=False):
            ...
            for instr in instruments:
                ...
                try:
                    with open(
                        os.path.join(inst_dir, "motion_features.json")
                    ) as f:
...
def compute_global_correlations(
    motion_features, audio_features, corr_thresh=0.25, pval_thresh=0.05
):
    speed_corrs = []
    accel_corrs = []
        ...
        for S in motion_features[A]:
            for instr in motion_features[A][S]:
                for part in motion_features[A][S][instr]:
                    ...
                    x2 = motion_features[A][S][instr][part][
                        "mean_acceleration"
                    ]
...
def sliding_correlation(signal1, signal2, window_size, step_size, threshold):
    ...
    return [
        (t, c)
        for t, c, p in zip(times, corrs, pvals)
        if not np.isnan(c)
        and not np.isnan(p)
        and abs(c) > threshold
        and p < 0.05
        ...
        for S in tqdm(motion_features[A], desc="Songs", leave=False):
            ...
            for instr in motion_features[A][S]:
                ...
                if os.path.exists(output_path):
                    print(
                        f"Strong windows already computed for {A}/{S}/{instr}, skipping"
                    )
                    continue
                strong_subset = {}
                for part in motion_features[A][S][instr]:
                    ...
                    speed_strong_windows = sliding_correlation(
                        x_sp, y, win, step, thresh
                    )
                    x_ac = motion_features[A][S][instr][part][
                        "mean_acceleration"
                    ]
                    acceleration_strong_windows = sliding_correlation(
                        x_ac, y, win, step, thresh
                    )
                    strong_subset[part] = {
                ...
                print(
                    f"Saved strong windows for {A}/{S}/{instr} to {output_path}"
                )
        ...
        for S in strong_windows[A]:
            for instr in strong_windows[A][S]:
                for part in strong_windows[A][S][instr]:
                    num_windows += len(
                        strong_windows[A][S][instr][part]["speed"]
